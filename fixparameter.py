...
class fixparameter():
    ...
    def add_by_func(self,func):
        ...
        for k,v in x.parameters.items():
            if v.default is not inspect.Parameter.empty :
                self._plist.append({'name':k,'val':v.default,'fixed':False})
            else :
                self._plist.append({'name':k,'val':0.0,'fixed':False})        
        self._calc_reduced()
    ...
    def list_map_reduced(self,mylist):        
        return [mylist[k] for k,p in enumerate(self._plist) if p['fixed'] == False]
...
if __name__ == "__main__":
    ...
    # TODO:
    # ff.x1 = 12.2 does not work as expected!!!
    ...
    mylist = [1,2,3]
    print(ff.list_map_reduced(mylist))
